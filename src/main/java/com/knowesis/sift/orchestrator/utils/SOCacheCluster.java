...
public class SOCacheCluster {
	...
	@PropertyInject("{{so.cache.password}}")	
	private String soCachePassword;
	...
	public void onPostConstruct() {
		...
		log.debug("inside on post construct. going to initialize jc");
		GenericObjectPoolConfig gopc = new GenericObjectPoolConfig();
		log.debug("default max total configuration attributes for pools created "+gopc.getMaxTotal());
		gopc.setMaxTotal(Integer.parseInt(System.getenv("REDIS_POOL_SIZE")));
		log.debug("new value for max total configuration attributes for pools created "+gopc.getMaxTotal());
		jc = new JedisCluster(nodes, 2000, 2000, 5, soCachePassword, gopc);
		log.debug("inside on post construct. initialized jc "+jc);
	}
	
//	public SOCacheCluster(){
//		/*  sample value for cluster nodes:
//        localhost:30001,localhost:30002,localhost:30003,localhost:30004,localhost:30005,localhost:30006 */
//		Set<HostAndPort> nodes = new HashSet<>();
//		String clusterNodes = System.getenv(Constants.SO_CACHE_HOSTS);
...
//		}
//		log.debug("inside on post construct. going to initialize jc");
//		jc = new JedisCluster(nodes, 2000, 2000, 5, System.getenv("SO_CACHE_PASSWORD"), new GenericObjectPoolConfig());
//		log.debug("inside on post construct. initialized jc "+jc);
//	}
	 ...
	 * </ol>
	 * @param headers Map. headers[SOCache.Command]. This specified the cache operation to be performed
	...
	private void cacheExpire(Map<String, Object> headers) {
		try {
			jc.expire((String)headers.get(Constants.SOCache_dot_Key), (int)headers.get(Constants.SOCache_dot_TTL));
		}catch(Exception e){
			log.error("Exception caught while cache EXPIRE operation");
		}
		
	}

	private boolean cacheExists(Map<String, Object> headers) {
		boolean ifKeyExists = false;
		try {
			log.debug("received cache EXISTS request.key {}", (String)headers.get(Constants.SOCache_dot_Key));
			ifKeyExists = jc.exists((String)headers.get(Constants.SOCache_dot_Key));
		}catch(Exception e) {
			e.printStackTrace();
			log.error("Exception caught while cache EXISTS operation  "+e+" "+jc);
		}
	...
	private void cacheSetExpire(Map<String, Object> headers) {
		log.debug("SETANDEXPIRE request received with headers {}", headers);
		jc.set((String)headers.get(Constants.SOCache_dot_Key),(String)headers.get(Constants.SOCache_dot_Value));
		jc.expire((String)headers.get(Constants.SOCache_dot_Key), (int)headers.get(Constants.SOCache_dot_TTL));
		log.debug("key {}, set and expire operation executed.", (String)headers.get(Constants.SOCache_dot_Key));
	}
